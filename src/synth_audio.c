...
f32* fill_phase_diff_buffer(f32* freq, grv_arena_t* arena) {
	f32* outptr = arena_alloc_buffer(arena);
	f32* dst = outptr;
	for (i32 i = 0; i < AUDIO_FRAME_SIZE; i++) {
		*dst++ = (*freq++) / GRVGM_SAMPLE_RATE;
	}
	return outptr;
}

f32* fill_phase_buffer(f32* phase_diff, f32* phase_state, grv_arena_t* arena) {
	f32* outptr = arena_alloc_buffer(arena);
	f32* dst = outptr;
	f32 ph = *phase_state;
	for (i32 i = 0; i < AUDIO_FRAME_SIZE; i++) {
